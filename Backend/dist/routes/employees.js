...
router.get("/dashboard", auth_js_1.auth, (0, role_js_1.requireRole)("MANAGER", "PROJECT_MANAGER"), async (req, res) => {
    ...
    for (let w = 0; w < 4; w++) {
        ...
        const weekTasks = trendTasks.filter((t) => t.dueDate >= start && t.dueDate <= end);
    ...
    const teamOverview = employees.map((emp) => {
        ...
        const hoursLogged = tasks.reduce((sum, t) => sum + (t.assignedHours || 0), 0);
...
router.get("/me", auth_js_1.auth, (0, role_js_1.requireRole)("OPERATOR"), async (req, res) => {
    const me = await db_1.default.employee.findUnique({
...
router.get("/performance", auth_js_1.auth, (0, role_js_1.requireRole)("MANAGER", "PROJECT_MANAGER"), async (req, res) => {
    try {
        ...
        const formatted = employees.map((emp) => ({
            ...
            completedTasks: emp.user.tasksAssigned.filter((t) => t.status === "DONE").length,
...
router.get("/:employeeId/performance", auth_js_1.auth, (0, role_js_1.requireRole)("MANAGER", "PROJECT_MANAGER"), async (req, res) => {
    try {
        ...
        // Rating out of 5
        const rating = completedTasks.length > 0
        ...
        for (let i = 4; i >= 0; i--) {
            ...
            const dayLabel = (0, date_fns_1.format)(date, "EEE"); // Mon, Tue...
            // Find tasks updated on this day
            const dayEffort = tasks
                .filter((t) => (0, date_fns_1.isSameDay)(new Date(t.updatedAt), date))
        ...
        for (let i = 3; i >= 0; i--) {
            const weekStart = (0, date_fns_1.startOfWeek)((0, date_fns_1.subWeeks)(new Date(), i));
            const weekEnd = (0, date_fns_1.endOfWeek)((0, date_fns_1.subWeeks)(new Date(), i));
            const count = tasks.filter((t) => t.status === "DONE" &&
                new Date(t.updatedAt) >= weekStart &&
                new Date(t.updatedAt) <= weekEnd).length;
