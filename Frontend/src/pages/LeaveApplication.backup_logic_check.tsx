...
const LeaveApplication = () => {
    ...
    // Balance Calculation
    const [balance, setBalance] = useState({ total: 10, used: 0, remaining: 10 });
    ...
    const calculateBalance = () => {
        ...
        leaves.forEach(leave => {
            if (leave.status === "APPROVED") {
                ...
                // Get all days in the leave interval
                const days = eachDayOfInterval({ start, end });

                // Count days that fall in the current month
                const daysInCurrentMonth = days.filter(day => isSameMonth(day, now));
                usedCount += daysInCurrentMonth.length;
            }
        });
        ...
        setBalance({
            total: TOTAL_LEAVES,
            used: usedCount,
            remaining: Math.max(0, TOTAL_LEAVES - usedCount)
        });
    };
                ...
                <div className="grid gap-4 md:grid-cols-3">
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-sm font-medium">Total Leaves (Month)</CardTitle>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold text-yellow-600">{balance.used}</div>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold text-green-600">{balance.remaining}</div>
                        </CardContent>
                    </Card>
                </div>
