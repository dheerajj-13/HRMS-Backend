...
import { Button } from "@/components/ui/button"; // Corrected Button import based on common practice
...
// --- SKELETON LOADER COMPONENTS (Adjusted for mobile sizing) ---
...
const SkeletonEmployeePerformanceDashboard = () => (
    <Layout>
        <div className="space-y-6 sm:space-y-8 p-4 sm:p-6 min-h-screen">
            {/* Skeleton Header and Total Employees Card - Stacked on Mobile */}
            ...
            <div className="flex flex-col gap-4 lg:grid lg:grid-cols-3 xl:grid-cols-4 lg:h-[calc(100vh-160px)]">
                ...
                {/* Detail View Skeleton - Responsive padding */}
                <Card
                ...
                >
                    ...
                    {/* Top Stats Skeleton - 2 columns on mobile */}
...
// --- Helper Components (Responsive updates applied) ---
            ...
            {statItems.map((item: any) => (
                ...
                >
                    <div className="space-y-0.5">
                        <p className="text-xs sm:text-sm font-medium text-gray-500">{item.title}</p>
                        <h3
                    ...
                    <div className="text-right">
                        {item.change && (
                            <div className={`text-[10px] sm:text-xs font-semibold ${item.trendColor}`}>
                                {item.change}
                            </div>
                        )}
                        <item.icon className={`h-5 w-5 sm:h-6 sm:w-6 opacity-70 ${item.color}`} /> {/* Reduced icon size */}
                    </div>
                </Card>
            ))}
        </div>
...
const PerformanceMatrix = ({ data }: { data: PerformanceData }) => (
    <Card className="p-4 sm:p-6 mb-4 sm:mb-6 border-[#0000cc]/20 shadow-sm">
        <CardTitle
            className={`flex items-center gap-2 mb-4 text-lg sm:text-xl`} // Reduced title size
            style={{ color: COLOR_PRIMARY }}
        >
            <Zap className={`h-4 w-4 sm:h-5 sm:w-5 ${COLOR_ACCENT_ICON}`} />
            Performance Matrix
        </CardTitle>
        <div className="h-[200px] sm:h-[300px]"> {/* Adjusted height to be even smaller on mobile (200px) */}
            <ResponsiveContainer width="100%" height="100%">
                <RadarChart outerRadius={window.innerWidth < 640 ? 80 : 130} data={data.radar}> {/* Reduced outer radius further */}
...
const SkillsAchievements = ({ data }: { data: PerformanceData }) => (
    ...
    <div className="flex flex-col lg:grid lg:grid-cols-2 gap-4 sm:gap-6">
        <Card className="p-4 sm:p-6 border-[#0000cc]/20 shadow-sm">
            <CardTitle className={`text-lg sm:text-xl mb-4`} style={{ color: COLOR_PRIMARY }}>
                Top Skills
            </CardTitle>
        ...
        <Card className="p-4 sm:p-6 border-[#0000cc]/20 shadow-sm">
            ...
            <div className="space-y-3 sm:space-y-4">
                {data.achievements.map((ach) => (
                    ...
                    >
                        <Trophy className={`h-5 w-5 sm:h-6 sm:w-6 ${COLOR_ACCENT_ICON} flex-shrink-0`} /> {/* Reduced icon size */}
                        <div>
                            <h4 className="font-semibold text-sm sm:text-base text-gray-800">{ach.title}</h4>
...
export default function EmployeePerformanceDashboard() {
    ...
    const xAxisWeeklyProps = window.innerWidth < 640 // Tailwind's 'sm' breakpoint is 640px
        ? { angle: -45, textAnchor: 'end', height: 40, style: { fontSize: 10 } }
        : { angle: 0, textAnchor: 'middle', height: 30, style: { fontSize: 12 } };
    
    // Apply smaller height/font for Line Chart XAxis
    const xAxisLineProps = window.innerWidth < 640 
    ...
    return (
        <Layout>
            <div className="space-y-6 sm:space-y-8 p-4 sm:p-6 min-h-screen">
                ...
                {/* Header and Total Employees Card - Stacked on Mobile */}
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b pb-3 sm:pb-4">
                    ...
                    {/* Total Employees Stat Card (Mini) - Fixed Size Reduction */}
                    <Card
                        className={`p-3 mt-3 sm:mt-0 shadow-md w-full sm:w-56`}
                        style={{
                            backgroundColor: `${COLOR_PRIMARY}1A`,
                    ...
                    >
                        <div className="flex items-center gap-2 sm:gap-3">
                            <Users className={`h-4 w-4 sm:h-6 sm:w-6 ${COLOR_ACCENT_ICON}`} /> {/* Reduced icon size */}
                            <div>
                                <div className="text-xs font-medium text-gray-600"> {/* Reduced text size */}
                                    Total Team Members
                                </div>
                                <div className="text-xl sm:text-2xl font-bold" style={primaryStyle}> {/* Reduced value size */}
                                    {employees.length}
                                </div>
                            </div>
                        </div>
                ...
                {/* Main Content Grid - STACKED on mobile (default) */}
                <div className="flex flex-col gap-6 lg:grid lg:grid-cols-3 xl:grid-cols-4 lg:h-[calc(100vh-160px)]">
                    
                    {/* Employee List */}
                    <Card
                        className={`lg:col-span-1 p-4 flex flex-col shadow-lg border-[#0000cc]/20 h-[300px] lg:h-full`} // Fixed height on mobile
                    >
                    ...
                    <div className="lg:col-span-2 xl:col-span-3 relative">
                        ...
                        {selectedEmployee && selectedEmployee.performance ? (
                            ...
                            >
                                <div className="mb-4 sm:mb-6 border-b pb-3">
                                    <h2 className="text-xl sm:text-3xl font-bold" style={primaryStyle}>
                                ...
                                <div className="flex flex-col lg:grid lg:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
                                    ...
                                    <Card className="p-4 sm:p-6 border-[#0000cc]/20 shadow-sm">
                                        <CardTitle
                                            ...
                                            style={primaryStyle}
                                        >
                                            <Clock className={`h-4 w-4 sm:h-5 sm:w-5 ${COLOR_ACCENT_ICON}`} />{" "}
                                            ...
                                            {selectedEmployee?.performance?.weeklyHours && (
                                                ...
                                                >
                                                    <CartesianGrid
                                                        ...
                                                        stroke="#e2e8f0"
                                                    />
                                                    <XAxis dataKey="day" stroke="#666" {...xAxisWeeklyProps} />
                                            ...
                                            {selectedEmployee?.performance?.completionTrend && (
                                                ...
                                                >
                                                    <CartesianGrid
                                                        ...
                                                        stroke="#e2e8f0"
                                                    />
                                                    <XAxis dataKey="week" stroke="#666" {...xAxisLineProps} />
                                                    <YAxis stroke="#666" style={{ fontSize: 10 }} />
                                                    <Tooltip
                                                        contentStyle={{
                                                            borderRadius: "0.5rem",
                            ...
                            <div className="h-full flex items-center justify-center bg-gray-50 rounded-lg border border-dashed border-gray-300 shadow-inner">
                                <div className="text-center text-gray-500 p-8">
                                    <Target
                                        ...
                                        style={primaryStyle}
                                    />
                                    <h3 className={`text-lg sm:text-xl font-semibold`} style={primaryStyle}>
                                        Performance View
                                    </h3>
                                    <p className="text-sm">
