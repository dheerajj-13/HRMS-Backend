...
const LeaveManagement = () => {
    ...
    const handleStatusUpdate = async (id: string, status: "APPROVED" | "DECLINED") => {
        try {
            ...
            // FIND THE TARGET LEAVE TO NOTIFY EMPLOYEE
            const targetLeave = updatedLeaves.find((l: any) => l.id === id);
            if (targetLeave && targetLeave.employeeId) {
                createNotification(targetLeave.employeeId, `Your leave request was ${status.toLowerCase()}.`, status === "APPROVED" ? "success" : "error");
            }
    ...
    const handleDownloadPDF = () => {
        ...
        // Table
        const tableColumn = ["Employee", "Department", "Reason", "Dates", "Status"];
        const tableRows: any[] = [];
