...
const StatusIcon = ({ status }: { status: Task["status"] }) => {
    switch (status) {
        ...
        case "DONE":
            return <CheckCircle2 className={`h-4 w-4 sm:h-5 sm:w-5 text-green-600`} />; // Responsive icon size
        case "TODO":
...
const SkeletonTaskHub = () => (
    <Layout>
        <div className="space-y-4 sm:space-y-8 p-3 sm:p-6"> {/* Reduced padding/spacing for mobile */}
            <Card
                                        ...
                                        selectedTask.comments.map((c: Comment) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    {!isOwnRole && (
                                                        <Check className={`h-4 w-4 mt-1 sm:mt-0 sm:ml-2 ${tickColor} flex-shrink-0`} />
                                                    )}
                                                </div>
                                            );
                                        })
        ...
        <CardContent className="p-0 space-y-4">
            <CardHeader className="p-0 pb-4">
                ...
                <CardTitle
                    ...
                    style={{ color: COLOR_PRIMARY }}
                >
                    <CalendarDays className={`h-4 w-4 sm:h-5 sm:w-5 ${COLOR_ACCENT_ICON}`} />
            ...
            {loading ? (
                ...
                <div className="space-y-4 sm:space-y-6"> {/* Reduced vertical space on mobile */}
                    {sortedDates.map((date) => (
                        ...
                        >
                            ...
                            <ul className="space-y-1"> {/* Reduced vertical space */}
                                {completedByDate[date].map((task) => (
                                    <li
                                        key={task.id}
                                        className="flex items-center gap-2 text-xs sm:text-sm text-green-800" // Reduced text size
                                    >
                                        <CheckCircle2 className={`h-3 w-3 sm:h-4 sm:w-4 text-green-600`} /> {/* Reduced icon size */}
...
export default function EmployeeTaskTimeline() {
    ...
    return (
        <Layout>
            <div className="space-y-4 sm:space-y-8 p-4 sm:p-6"> {/* Reduced padding/spacing for mobile */}
                <Card className={`p-4 sm:p-6 shadow-lg border-[${COLOR_PRIMARY}]/20`}> {/* Reduced padding */}
                    <Tabs
                        value={activeTab}
                        onValueChange={(value) =>
                    ...
                    >
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 border-b pb-3 sm:pb-4">
                            ...
                            >
                                ...
                                >
                                    <ListChecks
                                        className={`h-3 w-3 sm:h-4 sm:w-4 ${ // Smaller icon
                                            activeTab === "timeline"
                                                ? COLOR_ACCENT_ICON
                                                : "text-gray-600"
                                            }`}
